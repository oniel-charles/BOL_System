...
//$pdf->AddPage('P',array(215.9,177.8),0);
  ...
  if (!$new_receipt ){
    $date = new DateTime($service_result[0][date].' '.sprintf('%04d',$service_result[0][time]));
  }
...
//var_dump($service_result);
        ...
        if ($request[1]=='create'){
          if ($new_receipt){
            ...
            if($rec_array[currency]==1 ){
              ...
              $sql =$sql. " VALUES (".date('Ymd').",".date('hi').",'".mysql_real_escape_string($bill_rec->consignee_name)."',".round($receipt_total,2).",".$rec_array[currency].",".round($receipt_total,2).",".$claims['id'].",".$bill_rec->id.",0,".$us_rate->exchange_rate.",'".$user_id_num."')"; //TBC created by      
      //echo $sql;
     // continue;          
    ...
    $pdf->Cell( 100, 5,'Item Description ', 'B');
    $pdf->Cell( 60, 5,'Amount ', 'B',1,'R');
    $pdf->SetFont('Arial','',12);
    $total=0;  
    foreach($service_result as $value){
      $us_amt_str='';
      if ($currency_code=='JMD' && $value["item_currency"]==2){
        $us_amt_str='US$ '.$value["currency_amount"].' '.$roe_str;
      }
      $pdf->Cell( 90, 8,$value["description"]);
      $pdf->SetFont('Arial','',10);
      $pdf->Cell( 20, 8,$us_amt_str, 0,0,'C');
      $pdf->SetFont('Arial','',12);
      $pdf->Cell( 50, 8,number_format($value["amount"],2), 0,1,'R');
      $total +=$value["amount"];
    }
