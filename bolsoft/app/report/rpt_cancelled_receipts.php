...
$this->Cell(20,5,' Receipt No.',1,0,'C',true);
$this->Cell(15,5,' Date',1,0,'C',true);
$this->Cell(30,5,' Amount',1,0,'R',true);
$this->Cell(40,5,' Customer',1,0,'L',true);
...
if ($claims==null){    
 ...
 $company_rec= mysqli_fetch_object($company);	
  ...
  $item_str='';
  if (intval( $this->requestUrl[5])!=0){
       $item_str=' and d.charge_item_id='.intval( $this->requestUrl[5]);
  }
 
 $sql="select r.cancel_date,r.exchange_rate,r.receipt_total,x.user_name as cancel_by,u.user_name,r.payee,r.id,r.receipt_date from ((receipt as r  left join user_profile as u on r.created_by=u.id) left join user_profile as x on r.cancel_by=x.id) where cancelled=1 and r.receipt_date between ".$this->requestUrl[2]." and ".$this->requestUrl[3]." $item_str order by r.id ";
...
$s_date= new DateTime($this->requestUrl[3]) ;
$e_date= new DateTime($this->requestUrl[4]) ;
...
foreach($table_data as $current_rec){
  ...
  $pdf->Cell(15,5,$date->format('d/m/Y'),0,0,'C');
  $pdf->Cell(30,5,number_format($current_rec["receipt_total"],2),0,0,'R');  
  $pdf->Cell(40,5,substr($current_rec["payee"],0,27),0,0,'L');
