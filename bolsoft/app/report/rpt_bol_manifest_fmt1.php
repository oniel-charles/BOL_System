...
if ($claims==null){    
 ...
 //Get Voyage 
   ...
   //Get Items   
   $sql="SELECT sum(number_of_items) as total_items FROM `bill_of_lading_detail` WHERE billoflading_id in (select id from bill_of_lading as b where b.voyage_id=$voyage_id $filter)";
   $result = mysqli_query($mysqli,$sql);
...
if(!$details_data[$current_rec["id"]][0]){
  ...
  $pdf->MultiCell(50,5,$current_rec["consignee_name"], 0,'L');  
  $cAddressPos =$pdf->GetVerticalPosition();
  $pdf->SetVerticalPosition( $item_pos ); 
  $pdf->SetX(67); 
  $pdf->MultiCell(51,5,$current_rec["shipper_name"], 0,'L');  
  ...
  $x=$pdf->GetX();
  $pdf->MultiCell(43, 5,$details_data[$current_rec["id"]][0]["Description_of_goods"], 0,'L');  
  
  $furthestPos = $pdf->GetVerticalPosition();
  $pdf->SetVerticalPosition( $item_pos ); 
  $pdf->SetX($x+43);
  $measure=$details_data[$current_rec["id"]][0]["measure"];
  if($clean && $details_data[$current_rec["id"]][0]["measure_unit"]=='cuf'){
    $measure=number_format($measure/35.315,2);
  }
  if(!$clean && $details_data[$current_rec["id"]][0]["measure_unit"]=='cum'){
    $measure=number_format($measure*35.315,2);
  }  
  $pdf->Cell(17,5,$measure,0,0,'R');
  $weight=$details_data[$current_rec["id"]][0]["weight"];
  if($clean && $details_data[$current_rec["id"]][0]["weight_unit"]=='lb'){
    $weight =number_format($weight/2.205,2);
  }
  if(!$clean && $details_data[$current_rec["id"]][0]["weight_unit"]=='kg'){
    $weight =number_format($weight*2.205,2);
  }
  $pdf->Cell(17,5,$weight,0,1,'R');
  $total=0;
  ...
  for ($i = 1; $i < count($details_data[$current_rec["id"]]); $i++) {       
    ...
    $x=$pdf->GetX();
    $item_pos = $pdf->GetVerticalPosition();
    $pdf->MultiCell(43, 5,$details_data[$current_rec["id"]][$i]["Description_of_goods"], 0,'L');  
    ...
    ***** */
    $measure=$details_data[$current_rec["id"]][$i]["measure"];
    if($clean && $details_data[$current_rec["id"]][$i]["measure_unit"]=='cuf'){
      $measure=number_format($measure/35.315,2);
    }
    if(!$clean && $details_data[$current_rec["id"]][$i]["measure_unit"]=='cum'){
      $measure=number_format($measure*35.315,2);
    }  
    $pdf->Cell(17,5,$measure,0,0,'R');
    $weight=$details_data[$current_rec["id"]][$i]["weight"];
    if($clean && $details_data[$current_rec["id"]][$i]["weight_unit"]=='lb'){
      $weight =number_format($weight/2.205,2);
    }
    if(!$clean && $details_data[$current_rec["id"]][$i]["weight_unit"]=='kg'){
      $weight =number_format($weight*2.205,2);
    }
    $pdf->Cell(17,5,$weight,0,1,'R');
    //************ */
