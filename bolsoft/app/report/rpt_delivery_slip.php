...
$order_number=0;
$reprint='';
   $user=$this->claims['full_name'];
   $sql="SELECT s.order_date,s.id,u.user_name,u.full_name FROM `delivery_order` as s left join user_profile as u on s.created_by=u.id where (cancelled is null or cancelled=0) and billoflading_id=".$this->requestUrl[2];
   ...
   if($order_number==0){
    ...
    date_default_timezone_set('America/Jamaica');
    $date= date('Ymd') ;
    $sql = "INSERT INTO `delivery_order` (`voyage_id`, `billoflading_id`, `created_by`,  `cancelled`, `order_date`, `cancel_by`, `cancel_date`)   VALUES ($rec->voyage_id, ".$this->requestUrl[2].", $user, 0, $date,'',0)";
    $result = mysqli_query($mysqli,$sql);
    ...
    $sql=" update bill_of_lading set order_processed=1 where id in (".$this->requestUrl[2].",".intval($rec->master_id).")";
    $result = mysqli_query($mysqli,$sql);
    if (!$result) {
      http_response_code(404);
      die($sql.'<br>'.mysqli_error($mysqli));
...
$pdf->Image('../img/company_icon.jpg',10,10,-300);
...
$pdf->Cell(150,5,$company_rec->company_name.' is not responsible for any theft or damage.','LR',1);
