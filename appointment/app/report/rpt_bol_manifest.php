...
$this->Cell(35, 5,$booking_rec->vessel_name, 0,1,'L');
$this->Cell(20, 5,'Voyage: ', 0,0,'R');
$this->Cell(40, 5,$booking_rec->booking_number, 0,1,'L');
$this->Cell(20, 5,'Reported Date: ', 0,0,'R');
$this->Cell(50, 5,$booking_rec->arrival_date->format('d/m/Y'), 0,1,'L');
$this->SetXY($x,$y);
$this->Cell( 150, 5,'', 0);
$this->Cell(1,5,'',0,1);
$this->SetFont('Arial','',9);
...
if ($claims==null){    
 ...
 $booking_id= $this->requestUrl[2];
   //Get Voyage 
   $sql="SELECT a.id,a.voyage_number,a.arrival_date,v.vessel_name FROM `booking` as a left join vessel as v on a.vessel_id=v.id WHERE a.id=$booking_id";
   $booking = mysqli_query($mysqli,$sql);
   $booking_rec= mysqli_fetch_object($booking);	
   $booking_rec->arrival_date= new DateTime($booking_rec->arrival_date);
   ...
   foreach($charges as $charge_rec){ 
    ...
    //containers 
    $sql="SELECT container_number,bill_of_lading_number FROM bill_of_lading as b left join `bill_of_lading_container` as c on b.id=c.billoflading_id where booking_id=$booking_id and parent_bol=1 ";
 ...
 $sql="SELECT b.id,m.master_bl, b.`bill_of_lading_number`,`consignee_fname`,`consignee_sname`,`consignee_address`,`consignee_phone_num`,`shipper_fname`,`shipper_sname`,`shipper_address`,`shipper_phone_num`,`notify_fname`,`notify_sname`,`notify_address`,`notify_phone_num` FROM `bill_of_lading` as b left join (select id,bill_of_lading_number as master_bl from bill_of_lading) as m on b.master_bol_id=m.id WHERE booking_id=$booking_id and b.parent_bol=0 order by m.master_bl,b.id " ;
