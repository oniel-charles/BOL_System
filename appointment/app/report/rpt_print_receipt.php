...
$this->SetY($y+5);
$this->SetX(73);
$this->Cell( 60, 5,$bill_rec->consignee_fname.' '.$bill_rec->consignee_sname, 0,1);
$this->SetY($y+10);
$this->SetX(73);	
        ...
        if ($this->requestUrl[3]=='create'){
          if ($new_receipt){
            ...
            if($rec_array[currency]==1 ){
              ...
              $sql =$sql. " VALUES ('".$payment_type_no."','".$payment_type."',".date('Ymd').",".date('his').",'".mysql_real_escape_string($bill_rec->consignee_fname).' '.mysql_real_escape_string($bill_rec->consignee_sname)."',".round($receipt_total,2).",".$rec_array[currency].",".round($receipt_total,2).",".$this->claims['id'].",".$bill_rec->id.",0,".number_format($us_rate->exchange_rate,2).",'".$user_id_num."')"; //TBC created by      
