...
function decodePassword($pass){
    ...
    for($i=0;$i<strlen($pass);$i++){
      $x=strpos(implode("",$cypher[($i % 2)]),$pass[$i]);
      //echo '<br> '.implode("",$cypher[($i % 2)]).'   '.$x;
      $val=$val.$x;
    }
    return $val;
  }
...
function login($mysqli) {
    ...
    $password =  base64_decode($mysqli->real_escape_string($_POST['password']));
     $pass=decodePassword($password);    
    // Using prepared statements means that SQL injection is not possible. 
    if ($stmt = $mysqli->prepare("SELECT id, bill_of_lading_number as  user_name, id as password,consignee_name as full_name 
        FROM bill_of_lading
       WHERE id = ? and bill_of_lading_number = ?
        LIMIT 1")) {
      ...
      // echo $stmt->num_rows.' - '.$user_id.' - '. $user_name.' - '. $db_password.' - '.$full_name;
        if ( $stmt->fetch()) {
           ...
           // if (checkbrute($user_id, $mysqli) == true) {
            ...
            echo ('<br><br>outy here'. $mysqli->error);
            // No user exists.
            return false;
        }
