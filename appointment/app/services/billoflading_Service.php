...
switch ($service_type) {
  ...
  // get bl info
   $sql="SELECT b.id,b.booking_id ,p.id as master_id,port.port_name,v.manifest_number,v.voyage_number,v.sail_date,b.bill_of_lading_number as refnum,p.bill_of_lading_number,b.consignee_fname,b.consignee_sname,b.consignee_address,b.shipper_fname,b.shipper_sname,b.shipper_address,s.vessel_name FROM ((((`bill_of_lading` as b left join booking as v on b.booking_id=v.id) left join vessel as s on v.vessel_id=s.id) left join port on b.port_of_delivery=port.id) left join (select id,bill_of_lading_number from bill_of_lading where parent_bol=1) as p on b.master_bol_id=p.id) WHERE b.id=".intval($bl_id);
 //  echo $sql;
   $result = mysqli_query($mysqli,$sql);
     ...
     case 'admin_update_assign':         
      ...
      case 'generate_charge':        
            ...
            for ($i=0;$i<mysqli_num_rows($rates);$i++) {              
              ...
              $sql="SELECT * FROM `bill_of_lading_detail`  WHERE  billoflading_id in (select id from bill_of_lading where booking_id=".$booking_id.") and package_type_id=".$rate->package_id." order by billoflading_id";
