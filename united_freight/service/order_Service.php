...
switch ($request[1]) {  
  ...
  case 'cancel':
          ...
          $sql=" update  bill_of_lading set order_processed=0 where id=".preg_replace('/[^a-z0-9_]+/i','',$request[3]);
          $result = mysqli_query($mysqli,$sql);
          if (!$result) { handleRollBack($mysqli);}          
          $sql=" update shipment_order set cancelled=1,cancel_date=".date('Ymd').",cancel_time=".date('hi').",cancel_by=".$claims['id']." where id=".preg_replace('/[^a-z0-9_]+/i','',$request[2]);          
          $result = mysqli_query($mysqli,$sql);      
