...
$order_number=0;
$reprint='';
   $user=$claims['user'];//$claims['full_name'];
   $sql="SELECT s.order_date,s.id,u.user_name,u.full_name FROM `shipment_order` as s left join user_profile as u on s.created_by=u.id where (cancelled is null or cancelled=0) and billoflading_id=".$request[0];
   $order = mysqli_query($mysqli,$sql);
   if (mysqli_num_rows( $order) >0){
       ...
       $reprint='REPRINT';
       $user=$order_rec->user_name;//$order_rec->full_name;
   }
...
//Get detail
$sql="SELECT measure,measure_unit,b.Description_of_goods,package.description, `number_of_items`,`weight`,`weight_unit` FROM `bill_of_lading_detail` as b  left join package on b.package_type_id=package.id WHERE b.billoflading_id=".$request[0];
...
$pdf->Image('../../img/company-icon.jpg',10,10,-300);
$pdf->Cell(10,10,'',0,0); $pdf->SetTextColor(0,0,255);
$pdf->Cell(65,10,'United Freight Forwarders',0,0);
