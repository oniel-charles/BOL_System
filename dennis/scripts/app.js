		...
		$.ajax({
			...
			success: function (data) {
				...
				$.each(data, function (key, value) {
					if (value.id > 0 || $this_select.hasClass("all-values")) {
						console.log('<option value=' + value.id + '>' + value.description + '</option>');
						$this_select.append('<option value=' + value.id + '>' + value.description + '</option>');
					}
				});
			},
...
function createLoginForm() {
	...
	domString += '</form> </div><br><i class="align-center" style="position:relative; margin-left:50px; cursor: pointer" onclick="alert(\'Contact Admin\')"> Forgot your password?</i> </div></div></div></div>';
...
function changePassword(){
	var $url = './api.php/change_password';
	var form_data = {};
...
function loadFormData($form, $model) {
	...
	for (var x in $model) {
		...
		if (field.hasClass("selectpicker with-ajax")) {
			...
			var xoptions = setAjaxSelectOptions('./api.php/package/api/search', 'Package Type');
...
function showMediumReportEmbed($url, $fileName, $header, p_iframe, rpt_data) {	
	...
	$.ajax({
		...
		success: function (response, status, xhr) {
				var iframe = $iframe;
				iframe.attr('src', './report'+response + '#page=1&zoom=75');			
...
function loadOptions() {
	...
	$.ajax({
		url: './api.php/menu_item/api/select',
		type: 'GET',
		dataType: 'json',
		success: function (data) {
			var option = data;
...
if (token) { 
	...
	if (!runcode && (opt!='index.html' && opt!='') && opt!='CRUDcreator.html'  && opt!='tempCrud.html') {
		//console.log(token);
		//console.log(opt);
		//console.log(opt_list); SECURE
		window.location = './index.html';
		token = ''; saveToken();
	}
	/* TBC
	$.ajax({
		url: './service.php/load/' + window.location.pathname.split('/').slice(-1)[0],
		type: 'GET',
		dataType: 'text',
		async: false,
		success: function (data) {
...
function getBusinessObject(table,id) {
	var path_arr=table.split('/');
	var path=table; //path_arr[0];
	//if(path_arr.length>1){
	//	path=path+'/api';
	//	for(var i=1;i<path_arr.length;i++){path=path+'/'+path_arr[i];}
	//}
	if(id){id='/'+id;}
	else  {id='';    }
	var rtn_data;
