...
if ($claims==null){
  ...
  if ($rec->data_value=='mby'){
    require_once './report/rpt_print_batch_order_mby.php';
  }else{    
    if(file_exists('./report/rpt_print_batch_order_kgn.php')){
    require_once './report/rpt_print_batch_order_kgn.php';  
    }else{
      echo getcwd()." no existOHC is here - ".$rec->data_value.' aa ';
    }
  }  
  exit();
  ?>