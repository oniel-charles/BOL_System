...
if ($claims==null){    
 ...
 $company_rec= mysqli_fetch_object($company);	
  ...
  if(intval($post_data["userid"])>0){
    $item_str=$item_str.' and r.created_by='.intval($post_data["userid"]);
  }
  $item_str=$item_str.' and d.charge_item_id in ('.implode("," ,$post_data["charge_ids"]).') and r.currency_id ='.$post_data["currency_id"];
 
 $sql="select r.exchange_rate,d.currency_amount,u.user_name,r.payee,r.id,r.receipt_date,d.amount,c.currency_id,c.description from (((((receipt as r left join receipt_detail as d on r.id=d.receipt_id ) left join charge_item as c on d.charge_item_id=c.id) left join user_profile as u on r.created_by=u.id) left join bill_of_lading as b on r.billoflading_id=b.id) left join port as p on b.port_of_loading=p.id) where  (r.cancelled is null or r.cancelled=0)  and r.receipt_date between ".$this->requestUrl[2]." and ".$this->requestUrl[3]." $item_str order by c.description,r.receipt_date ";
//echo $sql;
