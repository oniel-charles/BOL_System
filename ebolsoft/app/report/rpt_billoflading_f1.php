...
if (isset($this->requestUrl[3])){
  ...
  $sql="SELECT b.id,m.master_bl, b.`bill_of_lading_number`,concat(consignee_fname,' ',consignee_sname,' ',consignee_other_name) as consignee_name,`consignee_address`,`consignee_phone_num`,concat(shipper_fname,' ',shipper_sname,' ',shipper_other_name) as shipper_name,`shipper_address`,`shipper_phone_num`,concat(notify_fname,' ',notify_sname,' ',notify_other_name) as notify_name,`notify_address`,`notify_phone_num` FROM `bill_of_lading` as b left join (select id,bill_of_lading_number as master_bl from bill_of_lading) as m on b.master_bol_id=m.id WHERE voyage_id=".$this->requestUrl[3]." and b.parent_bol=0 order by m.master_bl,b.id " ;
...
// excecute SQL statement
  $sql="SELECT  b.value_of_goods,p.id as master_id,concat(p.consignee_fname,' ',p.consignee_sname,' ',p.consignee_other_name)  as master_name,p.consignee_address as master_address,pl.port_name as loading,pd.port_name as discharge,v.voyage_number,v.sail_date,b.bill_of_lading_number as refnum,p.bill_of_lading_number,concat(b.notify_fname,' ',b.notify_sname,' ',b.notify_other_name) as notify_name,b.notify_address,concat(b.consignee_fname,' ',b.consignee_sname,' ',b.consignee_other_name) as consignee_name,b.consignee_address,concat(b.shipper_fname,' ',b.shipper_sname,' ',b.shipper_other_name) as shipper_name,b.shipper_address,s.vessel_name  FROM (((((`bill_of_lading` as b left join booking as v on b.booking_id=v.id) left join vessel as s on v.vessel_id=s.id) left join port  as pd on b.port_of_discharge=pd.id) left join port  as pl on b.port_of_loading=pl.id) left join (select id,bill_of_lading_number,consignee_fname,consignee_other_name,consignee_sname,consignee_address from bill_of_lading where parent_bol=1) as p on b.master_bol_id=p.id) WHERE b.id=".$bl_ids[$j];
...
$chgarr = array();
$chgarr['FRT']=0;
$sql="SELECT b.amount,b.amount_paid,c.item_code,c.description,b.charge_item_id FROM bill_of_lading_other_charge as b left join charge_item as c on b.`charge_item_id`=c.id where b.charge_item_id=".$freight->data_value." and b.billoflading_id=".$bl_ids[$j];
$result = mysqli_query($mysqli,$sql);
...
for ($i=0;$i<mysqli_num_rows($details);$i++) {
    ...
    $pdf->SetY($y);
    $pdf->SetX(168);
    $total_measure+=$detail_rec->measure;
    $total_weight+=$detail_rec->weight;
    $pdf->Ln();
...
$y=$pdf->GetY();
$x=$pdf->GetX();
$pdf->Cell(50,7,'Freight : '.number_format($charge->amount-$charge->amount_paid,2).' USD ',0,1);
$pdf->SetX($x);
$pdf->Cell(17,7,'Weight : '.$total_weight.' KG',0,1);
$pdf->Cell(30,7,'Port of Loading : ',0,0);
$pdf->Cell(60,7,$rec->loading,0,0);
$pdf->SetX($x);
$pdf->Cell(17,7,'Measurement : '.number_format($total_measure/35.315,2).' CBM',0,1);
$pdf->Ln();
