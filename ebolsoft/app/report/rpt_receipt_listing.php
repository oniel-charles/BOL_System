...
if ($claims==null){    
 ...
 (((((((receipt as r left join receipt_payment as p on r.id=p.receipt_id ) left join bill_of_lading as b on r.billoflading_id=b.id) left join booking as v on b.booking_id=v.id) left join port as po on v.port_of_loading=po.id) left join user_profile as u on r.created_by=u.id) left join currency as c on r.currency_id=c.id) left join (select amount as freight_amount,receipt_id,charge_item_id from receipt_detail where charge_item_id=-999) as d on r.id=d.receipt_id)  where (r.cancelled is null or r.cancelled=0) and r.receipt_date between ".$this->requestUrl[2]." and ".$this->requestUrl[3]."  $filter_str   $order_by";   
