...
if ($claims==null){    
 ...
 $sql="select avg(r.receipt_total) as amt ,r.exchange_rate,sum(d.currency_amount) as currency_amt,u.user_name,r.payee,r.id,r.receipt_date,sum(d.amount) as dtl_sum from (((receipt as r left join receipt_detail as d on r.id=d.receipt_id ) left join charge_item as c on d.charge_item_id=c.id) left join user_profile as u on r.created_by=u.id) where (r.cancelled is null or r.cancelled=0) and r.receipt_date between  ".$this->requestUrl[2]." and ".$this->requestUrl[3]." group by r.currency_id,u.user_name,r.payee,r.id,r.receipt_date  $order_by";   
