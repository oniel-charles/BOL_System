...
switch ($this->requestUrl[1]) {
    case 'select':    
    $sql="SELECT d.receipt_id as receipt_number,d.amount,c.description,amount as paid,d.id FROM `receipt_detail` as d left join charge_item as c on charge_item_id=c.id WHERE d.receipt_id=".preg_replace('/[^a-z0-9_]+/i','',$this->requestUrl[2]);
    break;
    case 'excel':    
